$(document).ready(function () {
    ...
    $('#btnRecherche').on('click', function (e) {
        ...
        if (!codeassurance) {
            Swal.fire({
                text: 'Veuillez saisir le code assurance.',
                icon: 'warning',
                confirmButtonText: 'Ok',
                customClass: {
                    confirmButton: 'kt-btn fw-bold kt-btn-primary',
        ...
        $("iframe[data-rapport='titrerecetteparassurance']").remove();

        $.ajax({
            url: '/Rapport/GenererTitreRecetteParAssurance',
            type: 'GET',
            data: { debut: debut, fin: fin, codeassurance: codeassurance },
            beforeSend: function () {
                $('#loader-spinner').show();
            },
            success: function (data) {
                if (data.success) {
                    ...
                    printFrame.setAttribute('data-rapport', 'titrerecetteparassurance');
                    ...
                    //    var doc = printFrame.contentWindow || printFrame.contentDocument;
                    //    if (doc.document) doc = doc.document;
                    //    doc.body.innerHTML = '<img src="' + data.imageBase64 + '" style="max-width:100%;"/>';
                    //    setTimeout(function() {
                    //        printFrame.contentWindow.focus();
                    //        printFrame.contentWindow.print();
                    //        setTimeout(function() {
