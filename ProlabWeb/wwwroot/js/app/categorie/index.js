...
var KTCategorieList = function () {
    var table;
    var datatable;
    var loader;

    // Initialisation du DataTable
    var initCategorieTable = function () {
    ...
    var handleSearchDatatable = function () {
        const filterSearch = document.querySelector('[data-kt-categorie-table-filter="search"]');
        if (filterSearch) {
    ...
    var handleDeleteRows = function () {
        const deleteButtons = document.querySelectorAll('[data-kt-categorie-table-filter="delete_row"]');
        deleteButtons.forEach(d => {
            d.addEventListener('click', function (e) {
                ...
                Swal.fire({
                    text: "Voulez-vous vraiment supprimer " + categoryName + " ?",
                    icon: "warning",
                    ...
                    if (result.value) {
                        let id = $(d).data('id');
                        console.log('Suppression categorie id :', id); // LOG
                        // Récupérer le token anti-forgery
                        var token = $('input[name="__RequestVerificationToken"]').val();
                        $.ajax({
                            url: '/Categorie/DeleteAjax',
                            type: 'POST',
    ...
    return {
        init: function () {
            console.log('KTCategorieList.init appelé'); // LOG
            table = document.querySelector('#kt_table_categorie_index');
            if (!table) return;
            loader = document.querySelector('#kt_spinner_categorie_index');
            console.log('initCategorieTable appelé'); // LOG
            initCategorieTable();
            handleSearchDatatable();
            handlePrintCategorie();
            handleDeleteRows();
        }
    };
}();
...
document.addEventListener('DOMContentLoaded', function () {
    if (typeof KTCategorieList !== 'undefined') {
        KTCategorieList.init();
    }
});
