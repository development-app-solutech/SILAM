...
var KTAssuranceList = function () {
    var table;
    var datatable;
    var loader;

    // Initialisation du DataTable
    var initAssuranceTable = function () {
    ...
    var handleSearchDatatable = function () {
        const filterSearch = document.querySelector('[data-kt-assurance-table-filter="search"]');
        if (filterSearch) {
    ...
    var handleDeleteRows = function () {
        const deleteButtons = document.querySelectorAll('[data-kt-assurance-table-filter="delete_row"]');
        deleteButtons.forEach(d => {
            d.addEventListener('click', function (e) {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        let id = $(d).data('id');
                        console.log('Suppression assurance id :', id); // LOG
                        // Récupérer le token anti-forgery
                        var token = $('input[name="__RequestVerificationToken"]').val();
                        $.ajax({
                            url: '/Assurance/DeleteAjax',
                            type: 'POST',
    ...
    return {
        init: function () {
            console.log('KTAssuranceList.init appelé'); // LOG
            table = document.querySelector('#kt_table_assurance_index');
            if (!table) return;
            loader = document.querySelector('#kt_spinner_assurance_index');
            console.log('initAssuranceTable appelé'); // LOG
            initAssuranceTable();
            handleSearchDatatable();
            handlePrintAssurance();
            handleDeleteRows();
        }
    };
}();
...
document.addEventListener('DOMContentLoaded', function () {
    if (typeof KTAssuranceList !== 'undefined') {
        KTAssuranceList.init();
    }
});
