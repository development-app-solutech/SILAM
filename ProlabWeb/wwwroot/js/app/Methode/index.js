...
var KTMethodeList = function () {
    var table;
    var datatable;
    var loader;

    // Initialisation du DataTable
    var initMethodeTable = function () {
    ...
    var handleSearchDatatable = function () {
        const filterSearch = document.querySelector('[data-kt-methode-table-filter="search"]');
        if (filterSearch) {
    ...
    var handleDeleteRows = function () {
        const deleteButtons = document.querySelectorAll('[data-kt-methodes-table-filter="delete_row"]');
        deleteButtons.forEach(d => {
            d.addEventListener('click', function (e) {
                e.preventDefault();
                const parent = e.target.closest('tr');
                const methodeName = parent.querySelectorAll('td')[1].innerText;
                Swal.fire({
                    text: "Voulez-vous vraiment supprimer " + methodeName + " ?",
                    icon: "warning",
                    ...
                    if (result.value) {
                        let id = $(d).data('id');
                        console.log('Suppression méthode id :', id); // LOG
                        // Récupérer le token anti-forgery
                        var token = $('input[name="__RequestVerificationToken"]').val();
                        $.ajax({
                            url: '/Methode/DeleteAjax',
                            type: 'POST',
                            ...
                            success: function (response) {
                                ...
                                if (response.success) {
                                    ...
                                    Swal.fire({
                                        text: "Méthode supprimée avec succès!",
                                        icon: "success",
                                        buttonsStyling: false,
    ...
    return {
        init: function () {
            console.log('KTMethodeList.init appelé'); // LOG
            table = document.querySelector('#kt_table_methode_index');
            if (!table) return;
            loader = document.querySelector('#kt_spinner_methode_index');
            console.log('initMethodeTable appelé'); // LOG
            initMethodeTable();
...
document.addEventListener('DOMContentLoaded', function () {
    if (typeof KTMethodeList !== 'undefined') {
        KTMethodeList.init();
    }
});
