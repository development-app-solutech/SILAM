...
var KTTarifanalyseassuranceCreate = function () {
            ...
            d.addEventListener('click', function (e) {
                ...
                $.ajax({
                    ...
                    error: function (request, error) {
                        ...
                        console.error('🖨️ Erreur lors de l\'impression (CREATE):', error);
                    }
    ...
    var handleSearchDatatable = () => {
        ...
        if (filterSearch && datatable) {
            ...
            console.log('🔍 Search handler attached (CREATE)');
        } else {
            console.warn('⚠️ Search input or datatable not found (CREATE)');
        }
    }

    // Handle laboratory filter
    var handleLaboratoireFilter = () => {
        console.log('🔬 handleLaboratoireFilter called (CREATE)');
        ...
        setTimeout(() => {
            ...
            if (laboratoireSelect) {
                console.log('Adding change event listener to laboratoire select (CREATE)');
                
                // Support pour Select2 et sélecteur normal
                $(laboratoireSelect).on('change.laboratoireFilter', function() {
                    console.log('🔄 Laboratoire changed (CREATE):', this.value);
                    const idlaboratoire = this.value;
                    if (idlaboratoire && idlaboratoire !== '') {
                        loadAnalysesByLaboratoire(idlaboratoire);
                    } else {
                        console.log('⚠️ Empty laboratoire selected, clearing table (CREATE)');
                        clearAnalysesTable();
                    }
                });
                
                console.log('✅ Event listener attached successfully (CREATE)');
            } else {
                console.warn('⚠️ laboratoireSelect not found (CREATE)');
            }
        }, 500);
    }

    // Load analyses by laboratoire with advanced logic
    var loadAnalysesByLaboratoire = (idlaboratoire) => {
        console.log('loadAnalysesByLaboratoire called with idlaboratoire:', idlaboratoire);
        
        const tableBody = document.querySelector('#kt_table_tarifanalyseassurance_create tbody');
        if (!tableBody) {
        ...
        $.ajax({
            ...
            data: { 
                ...
                codeassurance: null  // En mode CREATE, pas de code assurance
            },
            success: function (analyses) {
                ...
                if (analyses && analyses.length > 0) {
                    ...
                    console.log('✅ Table rebuilt with', analyses.length, 'analyses and DataTable initialized');
                } else {
            ...
            error: function (xhr, status, error) {
                ...
                console.warn('⚠️ Erreur lors du chargement des analyses (CREATE)');
            },
            complete: function () {
    ...
    var initializeDataTable = () => {
        if (table && !$.fn.DataTable.isDataTable(table)) {
            ...
            $('.dt-search').css('display', 'none');
            console.log('🔄 DataTable initialized (Analyses)');
        }
    }
        ...
        priceInputs.forEach(input => {
            ...
            if (analyseInput) {
                ...
                if (input.value) {
                    ...
                    console.log(`💾 Saved price ${input.value} for analyse ${analyseId}`);
                }
            }
        });
    }
    ...
    var setupPriceAutoSave = () => {
        ...
        priceInputs.forEach(input => {
            ...
            input.addEventListener('blur', function() {
                ...
                if (this.value) {
                    ...
                    console.log(`💾 Auto-saved price ${this.value} for analyse ${analyseId}`);
                } else {
                    // Remove from storage if empty
                    delete globalPrices[analyseId];
                    console.log(`🗑️ Removed price for analyse ${analyseId}`);
                }
            });
        ...
        console.log(`🔗 Auto-save setup for ${priceInputs.length} price inputs (Analyses)`);
    }

    // Function to clear analyses table
    var clearAnalysesTable = () => {
        console.log('🧹 Clearing analyses table (CREATE)');
        const tableBody = document.querySelector('#kt_table_tarifanalyseassurance_create tbody');
        if (!tableBody) return;
        ...
        console.log('📝 Table cleared - Please select laboratory message displayed (CREATE)');
    }
    ...
    var handleDeleteRows = () => {
        ...
        console.log('⚠️ handleDeleteRows désactivé (CREATE) - SweetAlert retiré');
        return;
    }
    ...
    var initToggleToolbar = () => {
        ...
        if (deleteSelected) {
            deleteSelected.addEventListener('click', function () {
                console.log('⚠️ Delete selected désactivé (CREATE) - SweetAlert retiré');
                // Fonction désactivée pour éviter les conflits avec SweetAlert
                return;
            });
        }
    ...
    return {
        // Public functions  
        init: function () {
            console.log('KTTarifanalyseassuranceCreate.init() called');
            table = document.querySelector('#kt_table_tarifanalyseassurance_create');
            console.log('table found:', table);
            if (!table) {
                console.log('table not found, exiting init');
                return;
            }

            loader = document.querySelector('#kt_spinner_tarifanalyseassurance_create');
            console.log('loader found:', loader);

            initTarifanalyseassuranceTable();
            console.log('initTarifanalyseassuranceTable() completed');
            //initToggleToolbar();
            handleSearchDatatable();
            console.log('handleSearchDatatable() completed');
            handleLaboratoireFilter();
            console.log('handleLaboratoireFilter() completed');
            //handlePrintTarifanalyseassurance();
            //handleResetForm();
            //handleDeleteRows();
            //handleFilterDatatable();
            
            console.log('KTTarifanalyseassuranceCreate.init() completed successfully');
        }
    }
}();
...
//    console.log('Document ready, initializing KTTarifanalyseassuranceCreate');
//    KTTarifanalyseassuranceCreate.init();
//});
