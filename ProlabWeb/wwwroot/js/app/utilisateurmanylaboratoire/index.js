...
var KTUtilisateurmanylaboratoireList = function () {
    var table;
    var datatable;
    var loader;
    ...
    var handleSearchDatatable = function () {
        const filterSearch = document.querySelector('[data-kt-utilisateurmanylaboratoire-table-filter="search"]');
        if (filterSearch) {
    ...
    var handlePrint = function () {
        const printLinks = document.querySelectorAll('[data-kt-list-utilisateurmanylaboratoire-table-print="print_table"]');
        printLinks.forEach(d => {
            d.addEventListener('click', function (e) {
                e.preventDefault();
                $.ajax({
                    url: '/UtilisateurManyLaboratoire/PrintList',
                    type: 'POST',
                    beforeSend: function () {
    ...
    var handleDeleteRows = function () {
        const deleteButtons = document.querySelectorAll('[data-kt-utilisateurmanylaboratoire-table-filter="delete_row"]');
        deleteButtons.forEach(d => {
            d.addEventListener('click', function (e) {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        let id = $(d).data('id');
                        console.log('Suppression utilisateurmanylaboratoire id :', id); // LOG
                        // RÃ©cupÃ©rer le token anti-forgery
                        var token = $('input[name="__RequestVerificationToken"]').val();
                        $.ajax({
                            url: '/UtilisateurManyLaboratoire/DeleteAjax',
                            type: 'POST',
                            ...
                            success: function (response) {
                                ...
                                if (response.success) {
                                    ...
                                    Swal.fire({
                                        text: "Utilisateur laboratoire supprimé avec succès!",
                                        icon: "success",
                                        buttonsStyling: false,
    ...
    return {
        init: function () {
            console.log('KTUtilisateurmanylaboratoireList.init appelÃ©'); // LOG
            table = document.querySelector('#kt_table_utilisateurmanylaboratoire_index');
            if (!table) return;
            loader = document.querySelector('#kt_spinner_utilisateurmanylaboratoire_index');
...
document.addEventListener('DOMContentLoaded', function () {
    if (typeof KTUtilisateurmanylaboratoireList !== 'undefined') {
        KTUtilisateurmanylaboratoireList.init();
    }
});
