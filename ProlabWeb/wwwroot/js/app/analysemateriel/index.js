...
var KTAnalyseMaterielList = function () {
    var table;
    var datatable;
    var loader;

    // Initialisation du DataTable
    var initAnalyseMaterielTable = function () {
    ...
    var handleSearchDatatable = function () {
        const filterSearch = document.querySelector('[data-kt-analysemateriel-table-filter="search"]');
        if (filterSearch) {
    ...
    var handleDeleteRows = function () {
        const deleteButtons = document.querySelectorAll('[data-kt-analysemateriel-table-filter="delete_row"]');
        deleteButtons.forEach(d => {
            d.addEventListener('click', function (e) {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        let id = $(d).data('id');
                        console.log('Suppression analysemateriel id :', id); // LOG
                        // Récupérer le token anti-forgery
                        var token = $('input[name="__RequestVerificationToken"]').val();
                        $.ajax({
                            url: '/Analysemateriel/DeleteAjax',
                            type: 'POST',
                            ...
                            success: function (response) {
                                ...
                                if (response.success) {
                                    ...
                                    Swal.fire({
                                        text: "Analyse matériel supprimée avec succès!",
                                        icon: "success",
                                        buttonsStyling: false,
    ...
    return {
        init: function () {
            console.log('KTAnalyseMaterielList.init appelé'); // LOG
            table = document.querySelector('#kt_table_analysemateriel_index');
            if (!table) return;
            loader = document.querySelector('#kt_spinner_analysemateriel_index');
            console.log('initAnalyseMaterielTable appelé'); // LOG
            initAnalyseMaterielTable();
            handleSearchDatatable();
            handlePrintAnalyseMateriel();
            handleDeleteRows();
        }
    };
}();
...
document.addEventListener('DOMContentLoaded', function () {
    if (typeof KTAnalyseMaterielList !== 'undefined') {
        KTAnalyseMaterielList.init();
    }
});
